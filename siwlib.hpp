...
#define LIGHTERGRAY  CLITERAL(Color){ 230, 230, 230, 255 }

const int GAME_WIDTH = 1, GAME_HEIGHT = 1, PIXEL_SIZE = 1;
...
{
	return Vector2{(GetMousePosition().x + cam.target.x) * GAME_WIDTH / GetScreenWidth(), (GetMousePosition().y + cam.target.y) * GAME_HEIGHT / GetScreenHeight()};
}
...
struct Speed
{
	...
	Vector2 to_Vector2();
	float x(){return to_Vector2().x;}
	float y(){return to_Vector2().y;}
};
Speed::Speed()
{
	ang = 0;
...
{
	s = Speed(Vector2{v.x + s.to_Vector2().x, v.y + s.to_Vector2().y});
	return s;
}
